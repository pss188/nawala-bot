...
# ============================================
# Pilih proxy terbaik dari daftar Anda
PROXY_HOST = "43.218.124.29"
PROXY_PORT = 28950  # HTTP proxy dengan response tercepat (366ms)
...
logger.info(f"🔑 Proxy: {PROXY_HOST}:{PROXY_PORT} (HTTP)")
...
class TrustPositifChecker:
    ...
    def check_single_domain(self, domain):
        ...
        try:
            ...
            # Coba semua endpoint dengan retry
            for endpoint in self.api_endpoints:
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if is_blocked is True:
                                ...
                                logger.warning(f"⚠️ Response tidak jelas, retry...")
                                time.sleep(1)
                                continue
                        else:
                ...
                logger.warning(f"⚠️ Endpoint {endpoint} gagal, coba yang lain...")
                time.sleep(2)
            
            # Jika semua gagal, asumsi AMAN
            logger.warning(f"⚠️ {domain}: Semua endpoint gagal, asumsi AMAN")
            return False
    ...
    def _parse_response(self, data):
        ...
        try:
            ...
            # Format 3: {'Blocked': True/False}
            if 'Blocked' in data:
                return bool(data['Blocked'])
            
            # Format 4: {'result': {'status': 'blocked'}}
            if 'result' in data and isinstance(data['result'], dict):
                if 'status' in data['result']:
                    if str(data['result']['status']).lower() == 'blocked':
                        return True
                    return False
                if 'blocked' in data['result']:
                    return bool(data['result']['blocked'])
            
            # Format 5: {'data': {'status': 'blocked'}}
...
async def kirim_status():
    try:
        ...
        message = (
            ...
            f"🔑 **Proxy:** {PROXY_HOST}:{PROXY_PORT} (HTTP)\n"
...
async def main():
    ...
    print(f"🔑 Proxy: {PROXY_HOST}:{PROXY_PORT} (HTTP)")
    ...
    logger.info("📍 Mode: TrustPositif API")
